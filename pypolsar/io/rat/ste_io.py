...
[1](https://www.dlr.de/hr/en/desktopdefault.aspx/tabid-2326/3776_read-48006)
"""
...
try:
    ...
    warnings.warn(
        'Failed to import ET from "lxml.etree". The Py2XML class will not work.',
        ImportWarning,
    )
...
try:
    ...
    warnings.warn(
        "Failed ot import pkg_resources and/or mako. Writing geo-Envi-headers for RAT files will not work.",
        ImportWarning,
    )
...
def saverat(filename, array, **kwargs):
        ...
        if "shape" in kwargs:
            ...
            if "nchannel" in kwargs:
                ...
                if len(kwargs["shape"]) <= 2:
                    ...
                    self.nchannel = ctypes.c_int(
                        np.product(kwargs["shape"][2::])
                    )
        if "var" in kwargs:
...
class RatFile:
    ...
    def create(self, shape=None, header=None, **kwargs):
        ...
        n_bytes = (
            reduce(lambda x, y: np.int64(x) * np.int64(y), self.shape)
            * self.dtype.itemsize
        )
    ...
    def write(self, arr=[], **kwargs):
        ...
        **Keywords**:
        ...
        if "block" in kwargs:
            ...
            if "header" in kwargs:
                self._ioerror(
                    "The header should have been written prior to block "
                    "writting!"
                )
            ...
            if arr.ndim == 2:
                ...
                arr = temp
                offset = (
                    1000 + np.int64(self.shape[1]) * block[0] * arr.itemsize
                )

            else:
                offset = (
                    1000
                    + np.ravel_multi_index(block[::2], self.shape)
                    * arr.itemsize
                )
        ...
        # no block writing
            ...
            if "header" in kwargs:
                ...
                if "shape" in kwargs:
                    self.Header.Rat.idl_shape = (ctypes.c_int * 8)(
                        *kwargs["shape"][::-1]
                    )
                ...
                self.Header = RatHeader(
                    shape=(
                        arr.shape if "shape" not in kwargs else kwargs["shape"]
                    ),
            ...
            self.shape = self._get_shape()
            self.Header.Rat.ndim = ctypes.c_int(len(self.shape))
            self.Header.Rat.nchannel = ctypes.c_int(
                int(np.product(self.shape[2:]))
            )
            ...
            with open(self.filename, "wb") as lun:
                ...
                if lun.tell() > n_bytes_total:
                    warnings.warn(
                        "The size of the RAT file exceed! The array "
    ...
    def read(self, **kwargs):
        ...
        **Keywords**:
        ...
        ind = tuple(
            map(lambda x, y: slice(x, y, None), block[::2], block[1::2])
        )
    ...
    def append(self, arr):
        ...
        if len(self.shape) > 1 and (self.shape[1:] != arr.shape[1:]):
            self._ioerror(
                ...
                "the array %s don't correspond to each other!"
                % (str(self.shape[1:]), str(arr.shape[1:]))
            )
        ...
        with open(self.filename, "r+b") as lun:
            ...
            if lun.tell() > n_bytes_total:
                warnings.warn(
                    "The size of the RAT file exceed! The array is "
    ...
    def read_header(self):
        ...
        if self.version == 2.0:
            ...
            with open(self.filename, "rb") as lun:
                ...
                shape = np.fromfile(
                    file=lun, dtype=data_type, count=ndim
                ).tolist()
                shape = shape[::-1]
                ...
                lun.seek(offset, 1)
                info = (
                    np.fromfile(file=lun, dtype="B", count=80)
                    .tostring()
                    .rstrip()
                )
            ...
            self.Header = RatHeader(
                shape=shape, ndim=ndim, var=var, rattype=rattype
            )
    ...
    def write_geo_envi_header(self):
        tmpl = Template(
            resource_string(__package__ + ".templates", "envihdr.tpl")
        )
    ...
    def _check_block(self, block, **kwargs):
        ...
        if "arr" in kwargs:
            if len(block) // 2 != kwargs["arr"].ndim:
                self._ioerror(
                    "The dimensions of block do not correspond to the "
                    "dimensions of array!"
                )
            ...
            if block_not_shape:
                self._ioerror(
                    ...
                    " the shape of the array %s!"
                    % (str(block), str(kwargs["arr"].shape))
                )
        return block
    ...
    def _get_dtype(self):
        ...
        try:
            ...
            self._ioerror(
                "The data type is either not specified or " "not supported!"
            )
...
class Py2Xml:
    ...
    def __init__(self, root):
        ...
        if self.__root__ is None:
            raise ValueError(
                'Expected an "object" element below the root element!'
            )
    ...
    def __getparam__(self, name):
        p = [
            p
            for p in self.__root__.iterchildren("parameter")
    ...
    def params(self):
        return [
            p.attrib["name"] for p in self.__root__.iterchildren("parameter")
        ]
    ...
    def xml2val(v, t):
        ...
        try:
            ...
            print(
                'PY2XML WARNING: Unsupported data type "%s" encountered. Skipping!'
                % (type)
            )
            return None
    ...
    def val2xml(v, t, value):
        cdict = {
            ...
            int: (str, "long"),
            float: (str, "double"),
            complex: (lambda z: f"({z.real},{z.imag})", "complex"),
        }
        ...
        if vsize == 1 and not isinstance(value, Py2Xml):
            ...
            if isinstance(value, np.ndarray):
                t.attrib["length"] = " ".join(
                    [str(l) for l in value.shape[::-1]]
                )
                value = value.flat
            vtype = type(value[0])
            t.text = cdict[vtype][1]
            v.text = (
                "[" + ", ".join([cdict[vtype][0](val) for val in value]) + "]"
            )
    ...
    def __totree(self):
        ...
        ste_root.addprevious(
            ET.PI("xml-stylesheet", 'type="text/xsl" href="stexml.xsl"')
        )
