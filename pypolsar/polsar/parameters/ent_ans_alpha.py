...
def ent_ani_alp(ew, ev):
    ...
    print("Eigenvalues shape: {0}\t Eigenvectors shape: {1}".format(ew.shape, ev.shape))
...
def pseudo_probabilities(ew):
    ...
    assert np.allclose(np.sum(p[:,:,:],axis=2), 1) , 'Pseudo Probabilities should be 1 along axis 2'
    return p


def entropy(p):
    entropy_h = -np.sum(p * np.log(p) / np.log(p.shape[2]), axis=2)
    assert np.max(entropy_h) <= 1 and np.min(entropy_h) >= 0, 'Entopry should be in range 0 < H < 1' 
    return entropy_h
...
def alpha_angles(ev, p):
    ...
    assert np.allclose(np.sum(ev[:, :, 0, :].imag,axis=2), 0) , 'Eigenvector''s first element imag part should be almost zero.'
    alpha[:, :, :] = np.arccos(np.abs(ev[:, :, 0, :])) # cos of each top element of each vector
